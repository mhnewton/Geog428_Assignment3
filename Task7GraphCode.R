...
ggplot(popPlaces, aes(x=Population, y=NearHospital)) + #classify x and y, qualitative classes
  ...
  stat_smooth(data = popPlaces, aes(x=Population, y=NearHospital), method = lm, se = FALSE, formula = y ~ x) +
  labs(title = "City population vs. distance to nearest hospital in British Columbia", x = "City Population (log)", y = "Distance to Closest Hospital (log(m))",
       caption = "Figure 1: Scatterplot of populated places in British Columbia and the distance to the nearest hospital.") + #label plot, x axis, y axis
